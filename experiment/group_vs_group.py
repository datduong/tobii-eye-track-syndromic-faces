...
this_k = 20 # ! lower-->rough shape, higher-->more smooth
...
main_folder = '' # @main_folder is where we save all the data
...
for folder in slide_folders: 
  ...
  for SUFFIX in ['','OnSlide1']: 
    ...
    if len(SUFFIX)>0: 
      ...
      group_folder = [f for f in os.listdir(folder) if SUFFIX not in f]

    print (group_folder)

    for i in group_folder: 
      for j in group_folder: 
        if i==j: continue
        #
        ...
        script = re.sub('GROUP1',i,script)
        script = re.sub('GROUP2',j,script)
        # 
        time.sleep( 1 )
