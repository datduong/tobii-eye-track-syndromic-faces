...
# tobii_num = [2,4,6,8,9,11,12,14,15,17] # tobii_num = [2,3,4,6,11,14] np.arange(2,18)
...
for this_group in ['Group1']: # 'all','Group1', 'Group2', 'Group3', 'Group4'
  ...
  for tobii_index, this_tobii in enumerate(tobii_num) : # ! for each original_image 
    ...
    for tobii_choice in TOBII_CHOICE: 
      ...
      # ! WE DO NOT WANT TO OVERLAY THE HEATMAP ON THE ORIGINAL IMAGE
      # ! WE WILL USE THE SEGMENTED IMAGES INSTEAD 
      original_image = Image.open(os.path.join(face_seg_dir,'Slide'+str(match_tobii_to_powerpoint[this_tobii])+'.PNG')).convert("L")
      original_image = np.array(original_image)
      # original_image = np.where(original_image>100, original_image/10,original_image)
      original_image = Image.fromarray(original_image).convert('RGBA')
      
      for i,foreground in enumerate(masks) : # @mask is array of ImageObject
        blend_image = Image.blend (original_image, foreground, alpha=0.4)
        # blend_image = foreground 
        if i == 0: 
    ...
    grid.save(os.path.join(output_dir,this_group+'_'+str(this_tobii)+'-'+id_to_english[this_tobii]+"tobii_overlay_short_overlay_segface.png"))
