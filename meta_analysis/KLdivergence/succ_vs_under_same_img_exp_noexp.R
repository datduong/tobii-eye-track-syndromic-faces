...
folder_path = 'KL-symmetric/ExpvNonExpResult'


data_path = paste0(data_path,folder_path)

what_group_name = 'Successful' # Successful Underperforming
threshold_used = c('low','high')
file_name = c('Total-G1vG1-70.0-kl-name.csv',
              'Total-G1vG1-110.0-kl-name.csv')
...
for (index in c(1:length(file_name))){
  ...
  forest(res, slab=dat$condition, main=this_title, xlim=xlim, alim=alim, header=c('Image','KL [95% CI]')) 
